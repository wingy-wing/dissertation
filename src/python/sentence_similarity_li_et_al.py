...
def get_sims(story_dict, cast_no):
    this_cast_story = story_dict[cast_no]
    sim_dict = OrderedDict()
    i = 0
    while i < len(story_dict):
        key = 'cast' + `i`
        comparison_story = story_dict[key]
        sim_dict[key] = similarity(this_cast_story, comparison_story, False)
...
def main(argv):
    number1 = int(argv[0])
    number2 = int(argv[1])
    cast_no1 = 'cast' + `number1`
    cast_no2 = 'cast' + `number2`
